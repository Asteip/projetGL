...
public final class DBHandlerDpt {
	...
	private static volatile DBHandlerDpt instance = null;
	private static final Logger logger = Logger.getLogger(DBHandlerDpt.class);
	Statement stmt = null;
	 ...
	 *  Singleton
	 */
	private DBHandlerDpt() {
		super();
	}



  //recup le fichier, si il la pas le créer

	public static final DBHandlerDpt getInstance() {

		if (DBHandlerDpt.instance == null) {
			synchronized(DBHandlerDpt.class) {
				if (DBHandlerDpt.instance == null) {
					DBHandlerDpt.instance = new DBHandlerDpt();
				}
			}
		}
		return DBHandlerDpt.instance;
	}
